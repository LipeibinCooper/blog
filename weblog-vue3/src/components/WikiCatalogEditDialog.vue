  ...
  >
    ...
    >
      ...
      >
        ...
        <button
          type="button"
          class="hover:bg-gray-100 py-3 px-3 rounded-lg flex items-center w-full font-medium rtl:text-right text-gray-500 dark:border-gray-700 dark:text-gray-400"
        ...
        <ul v-if="catalog.children && catalog.children.length > 0">
          <VueDraggable ref="el" v-model="catalog.children" @end="onDragEnd">
            ...
            >
              ...
              <!-- 重命名 -->
              <span
                class="hover:bg-gray-200 rounded py-2 px-2 ml-2 mr-2"
...
function updateWikiCatalogsData () {
  updateWikiCatalogs({ id: currWikiId.value, catalogs: catalogs.value }).then(
    res => {
      // 响参失败，提示错误消息
