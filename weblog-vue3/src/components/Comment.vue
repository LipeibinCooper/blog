            ...
            >
              ...
              <div class="flex items-center gap-3 mt-5">
                ...
                <div>
                  <a
                    v-if="
                      childComment.website && childComment.website.length > 0
                    "
                  ...
                  >
                    <img
                      v-if="
                        childComment.avatar && childComment.avatar.length > 0
                      "
...
const onPublishCommentClick = () => {
  initFormCommentUserInfo()
  // 校验
  if (!commentForm.nickname || commentForm.nickname.length === 0) {
    showMessage('请填写 QQ 号或昵称', 'warning')
    return
  }
  if (
    !commentForm.mail ||
    commentForm.mail.length === 0 ||
    !emailRegex.test(commentForm.mail)
  ) {
...
const onReplyContentSubmit = () => {
  ...
  if (!replyContent.value || replyContent.value.length === 0) {
